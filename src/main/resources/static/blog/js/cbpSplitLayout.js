...
(function() {
	...
	const splitlayout = document.getElementById( 'splitlayout' ),
		leftSide = splitlayout.querySelector( 'div.intro > div.side-left > div.intro-content' ),
		rightSide = splitlayout.querySelector( 'div.intro > div.side-right > div.intro-content' ),
	...
	function init() {
		...
		leftSide.querySelector( 'div.intro-info' ).addEventListener( eventtype, function( ev ) {
			reset();
			classie.add( splitlayout, 'open-left' );
			loadContent('article',1);
		} );

		rightSide.querySelector( 'div.intro-info' ).addEventListener( eventtype, function( ev ) {
			reset();
			classie.add( splitlayout, 'open-right' );
			loadContent('article',0);
	...
	function loadContent(content,id){
		let ele;
		if(id == 0) ele = "article_content_right"
		else ele = "article_content_left"
		$('#'+ele).load("/blog/"+content+"/"+id,{'page':1,'limit':7})
	}
	init();
